          ...
          {items.map((item, i) => (
            <div key={item.id} className="group relative">
              ...
              <button onClick={() => void toggle(item)}
                className="absolute top-2 right-2 w-8 h-8 bg-white/90 backdrop-blur rounded-full flex items-center justify-center shadow-sm hover:bg-red-50 transition-colors border border-gray-100 z-10"
              >
                <X size={12} className="text-gray-400 hover:text-red-500" />
              </button>

              <Link href={item.href} className="block text-[13px] font-medium text-gray-800 hover:text-black line-clamp-2 mb-1">
                {item.name}
              </Link>
              <p className="text-[14px] font-bold text-gray-900 mb-4">₹{Number(item.price).toLocaleString('en-IN')}</p>
              
              {/* FIXED: Instead of Add to Cart, we redirect to select variant options */}
              <Link
                href={item.href}
                className="w-full flex items-center justify-center gap-2 py-3 bg-white border border-gray-200 text-[11px] font-bold tracking-widest uppercase text-gray-600 rounded-xl hover:bg-black hover:text-white hover:border-black transition-all duration-300"
              >
                Select Options <ArrowRight size={12} />
              </Link>
            </div>
          ))}
        </div>
